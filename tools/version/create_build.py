"""Script to create a new versioned build of mDNSWindows.

Updates the versioned input files
Prompts the user to review before committing and tagging

This script is meant to be run by an mDNSWindows developer on a development workstation. It
...
FILE_IN_PATHS = [
    ...
    os.path.join(FILE_TEMPLATE_DIR, 'WinVersRes.h.in'),
]
FILE_OUT_PATHS = [
    os.path.join('tools', 'version', 'current_version.txt'),
    os.path.join('tools', 'install', 'vars.wxi'),
    os.path.join('mDNSWindows', 'WinVersRes.h'),
]
...
def update_version_files(repo_root, version):
    ...
    for in_file, out_file in zip(in_file_handles, out_file_handles):
        for line in in_file.readlines():
            line = line.replace('@MDNSWINDOWS_VERSION_MAJOR@', str(version[0]))
            line = line.replace('@MDNSWINDOWS_VERSION_MINOR@', str(version[1]))
            line = line.replace('@MDNSWINDOWS_VERSION_PATCH@', str(version[2]))
            line = line.replace('@MDNSWINDOWS_VERSION_BUILD@', str(version[3]))
            line = line.replace('@MDNSWINDOWS_VERSION_STRING@', version_str)
            line = line.replace('@MDNSWINDOWS_VERSION_DATESTR@', today.strftime('%d %B %Y'))
            line = line.replace('@MDNSWINDOWS_VERSION_COPYRIGHT@', 'Copyright ' + str(today.year) + ' ETC Inc.')
            out_file.write(line)
        in_file.close()
        out_file.close()
...
def main():
    ...
    mdnswin_repo = git.Repo(repo_root)
    ...
    commit_and_tag(mdnswin_repo, new_version)
